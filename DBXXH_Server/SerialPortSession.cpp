...
DBXXH::SerialPortSession::SerialPortSession(const std::string& name, unsigned int baudRate): serialPort(ioService)
{
...
DBXXH::SerialPortSession::~SerialPortSession()
{
    if (serialPort.is_open())
        serialPort.close();
}

void DBXXH::SerialPortSession::RunService()
{
    if (!serialPort.is_open())
    {
        std::cout << "Serial Port Open Failed" << std::endl;
...
void DBXXH::SerialPortSession::StartAsyncRead()
{
    if (needRead == 0)
    {
        switch (state)
...
void DBXXH::SerialPortSession::DataProcess()
{
    switch (buffer[4])
    {
    case 0x12:
...
void DBXXH::SerialPortSession::write(std::string& buf, boost::system::error_code& ec)
{
